...
if (isset($_POST['cod_item_picking'])) {
    ...
    $query = "SELECT * FROM itenspicking WHERE cod_itemPicking = '$cod_item_picking' AND codTurma = '{$_SESSION['codTurma']}' AND Situacao = 'Na expedição'";
    $resultado = $conexao->query($query);

    if ($resultado && $resultado->num_rows > 0) {
        $UpdateSituaciao = "UPDATE itenspicking SET Situacao = 'Nas docas' WHERE cod_itemPicking = '$cod_item_picking' AND codTurma = '{$_SESSION['codTurma']}'";
        $executeUpdate = $conexao->query($UpdateSituaciao);

        if ($executeUpdate) {
            echo json_encode(array('success' => true, 'Docas' => 1, 'cod_item_picking' => $cod_item_picking));
        } else {
            echo json_encode(array('success' => false, 'message' => 'Falha ao registrar item como pego'));
        }
    } else {
        echo json_encode(array('success' => false, 'message' => 'Não foi possível concluir a consulta SQL'));
    }
} else {
